...
if (!function_exists('securityGeneralTraceUrl')) {
    function securityGeneralTraceUrl(array $changes = []): string
    {
        $query = array_merge($_GET, $changes);
        foreach ($query as $key => $value) { if ($value === '' || $value === null) unset($query[$key]); }
        return '/helpdesk-php/admin-security-general.php' . ($query ? '?' . http_build_query($query) : '');
    }
}
$title = 'Eventos generales del sistema';
$activePage = 'system-security';
$pageTitle = 'Eventos generales';
$pageSubtitle = 'Acciones administrativas, técnicas y automáticas separadas de los contactos empresariales.';
